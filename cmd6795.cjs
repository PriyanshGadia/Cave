...
(async () => {
  ...
  await page.evaluate(async () => {
    ...
    // Disable bloom & color grading for clean neutral clay
    if (window.__lab.composer) {
      window.__lab.composer.passes.forEach(p => {
        if (p.constructor.name.includes('Bloom') || p.name?.includes('Bloom')) p.enabled = false;
      });
    }

    const g = window.__lab.sectorGroups['LS1'];
    const table = g.parent;
    const lab = window.__lab.scene;

    // Hide all siblings of g in table (the circular railing, other sector hitboxes, central pedestal)
    if (table && table.children) {
      table.children.forEach(c => {
        if (c !== g) c.visible = false;
      });
    }

    // Hide all objects in lab except table and lights
    lab.children.forEach(c => {
      if (c !== table && !c.isLight) c.visible = false;
    });

    // Clean g: remove old holographic children
    ...
    // Turn off pedestal cyan emitter materials
  ...
  fs.writeFileSync('C:/Users/gadia/.gemini/antigravity-ide/brain/694bd3af-9386-4a86-8cb6-98cd73f0a84a/test_studio_front.png', buf);
  console.log('Saved test_studio_front.png');
  await browser.close();
})();
